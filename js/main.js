( function ( $ ) {
	
	$(function(){
		var navUrl= document.URL;
		if(navUrl == 'http://www.es-designer.be/travaux/') {
			$('#tous').parent().attr('class', 'current');
		} 
		if(navUrl == 'http://www.es-designer.be/techniques/print/') {
			$('#print').parent().attr('class', 'current');
		} 
		if (navUrl == 'http://www.es-designer.be/techniques/web/') {
			$('#web').parent().attr('class', 'current');
		}	

		$("#contactName").focus();
		
		/* Bouton revenir en haut de la page */
		$('body').prepend('<a href="#top" class="top_link" title="Revenir en haut de page"><img src="http://www.es-designer.be/wp-content/themes/es/images/backToTop.png" alt="Haut de page" /></a>');  
		$(window).scroll(function(){  
			posScroll = $(document).scrollTop();  
			if(posScroll >=550)  
				$('.top_link').fadeIn(600);  
			else  
				$('.top_link').fadeOut(600);  
		});
	

		$.fn.heplbox = function( settings ) {

			var defaultSettings = {
					loaderGifURL: "data:image/gif;base64,",
					
					cssPrefix: "heplbox-",
					overlayOpacity: 0.66,
					zIndexBase: 9999
				};
			/* variable globale */
			var settings = $.extend(defaultSettings, settings), /*  sert ? mixer les variables, on prend les settings par default pour les remplcer par les settings */ 
			elements = [], /* stock dans un tableau nos liens */
			currentIndex = 0,
			$overlay, /* $ = variable jquery */
			$conteneur;
			
			/* m?thodes */
			var setup = function(){
			
				$overlay = $( 'body > div.' + settings.cssPrefix + 'overlay' ); // reference vers un div overlay
				$conteneur = $( 'body > div.' + settings.cssPrefix + 'conteneur' ); // refence vers un div conteneur
				
				/* on regarde si ces variables ($overlay, $conteneur) sont vides */
				if (!$overlay.size()) {
					
					// on cr?e un element et on ajoute
					$overlay = $('<div class="'+settings.cssPrefix+'overlay"></div>').css({
																							background: '#000000',
																							position: 'absolute',
																							top: 0,
																							left: 0,
																							zIndex: settings.zIndexBase,
																							cursor: 'pointer',
																							width: $(document).outerWidth(true),
																							height: $(document).outerHeight(true)
																							}).hide().appendTo('body').on('click.heplbox', closeBox);
																							
					$conteneur = $('<div class="'+settings.cssPrefix+'conteneur"></div>').css({
																							background: '#111111 url("'+settings.loaderGifURL+'") center no-repeat',
																							width: 25,
																							height: 25,
																							position: 'absolute',
																							zIndex: settings.zIndexBase + 1,
																							borderRadius: 2.5,
																							boxShadow: '0 0 10px rgba(0, 0, 0, 0.5)',
																							left: '50%',
																							marginLeft: -12.5,
																							top: ($(window).height()-50)/2
																							}).hide().appendTo('body');			
						
				}
			
			};
			
			var elementClicked = function(e){
			
				e.preventDefault(); // il empeche l'execution du comportement par d?fault
				// quel index correspond a quel url ==> indexOf retourn le 1er element
				currentIndex = elements.indexOf( $(this).attr('href') );
				openBox();
			};
			
			
			var openBox = function(){
				
				// on scroll tout en haut de la page
				$(window).scrollTop(0);
				
				// on affiche notre overlay
				$overlay.fadeTo('fast', settings.overlayOpacity);
				
				//on affiche notre conteneur
				$conteneur.fadeIn('normal', function(){ loadImage(); });
		
			};
			
			
			var loadImage = function(){
			
				var $img = $('<img src="'+elements[currentIndex]+'" />').css({ display: 'inline-block', borderRadius: 5, cursor: 'pointer' }).hide().appendTo('body').load(function(){
												$conteneur.animate({
																	padding: 5,
																	width: $img.width(),
																	height: $img.height(),
																	marginLeft: -($img.width()/2),
																	top: ($img.height() < $(window).height()) ? ($(window).height() - $img.height()) / 2 : 10
																	}, "fast", function(){
																							$img.remove().appendTo($conteneur).fadeIn('fast').on('click.heplbox', nextImage);
																						});
												});
			
			};
			
			
			var closeBox = function(){
									$conteneur.fadeOut('fast');
									$overlay.fadeOut('normal', function(){
																			$conteneur.find('img').remove();
																		});
									};
									
			var nextImage = function(){
										if(elements.length === 1)
											return closeBox(); 
											
											currentIndex = currentIndex + 1 < elements.length ? currentIndex + 1 : 0;
											$conteneur.find('img').fadeOut('fast', function(){
																								$conteneur.find('img').remove();
																								loadImage();
																							});
									};

			setup();

		
			return this.each(function(){
				elements.push($(this).attr('href')); // rechercher les attributs href et on stock dans le tableau elements
				$(this).on('click.heplbox', elementClicked);
				
				/* un espace de nom = c'est une espace virtuelle (tous nos objets, evenements) sont prefix? par un nom  */
			}); 
		};

	

		$(document).ready(function() {
			$('form#contactForm').submit(function() {
				$('form#contactForm .error').remove();
				var hasError = false;
				$('.requiredField').each(function() {
					if(jQuery.trim($(this).val()) == '') {
						var labelText = $(this).prev('label').text();
						$(this).parent().append('<span class="error">Vous avez oublié de rentrer votre  '+labelText+'.</span>');
						hasError = true;
					} else if($(this).hasClass('email')) {
						var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
						if(!emailReg.test(jQuery.trim($(this).val()))) {
							var labelText = $(this).prev('label').text();
							$(this).parent().append('<span class="error">Le champ est invalide '+labelText+'.</span>');
							hasError = true;
						}
					}
				});
				if(!hasError) {
					$('form#contactForm li.buttons input').fadeOut('normal', function() {
						$(this).parent().append('<img src="/wp-content/themes/td-v3/images/template/loading.gif" alt="Loading&hellip;" height="31" width="31" />');
					});
					var formInput = $(this).serialize();
					$.post($(this).attr('action'),formInput, function(data){
						$('form#contactForm').slideUp("fast", function() {				   
							$(this).before('<p class="thanks"><strong>Merci!</strong> Votre message a été envoyé avec succès.</p>');
						});
					});
				}
				
				return false;
				
			});
		});
	});
}( jQuery ) );
		